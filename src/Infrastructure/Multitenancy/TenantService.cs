...
using MCH.Application.Common.Exceptions;
using MCH.Application.Common.Persistence;
using MCH.Application.Multitenancy;
using MCH.Infrastructure.Persistence;
using MCH.Infrastructure.Persistence.Initialization;
using Mapster;
...
{
    private readonly IMultiTenantStore<MCHTenantInfo> _tenantStore;
    ...
    public TenantService(
        IMultiTenantStore<MCHTenantInfo> tenantStore,
    ...
    public async Task<string> CreateAsync(CreateTenantRequest request, CancellationToken cancellationToken)
    {
        ...
        var tenant = new MCHTenantInfo(request.Id, request.Name, request.ConnectionString, request.AdminEmail, request.Issuer);
    ...
    private async Task<MCHTenantInfo> GetTenantInfoAsync(string id) =>
        await _tenantStore.TryGetAsync(id)
            ?? throw new NotFoundException(_t["{0} {1} Not Found.", typeof(MCHTenantInfo).Name, id]);
}