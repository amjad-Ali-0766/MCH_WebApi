...
internal partial class UserService
{
    ...
    public async Task<string> AssignRolesAsync(string userId, UserRolesRequest request, CancellationToken cancellationToken)
    {
        ...
        if (await _userManager.IsInRoleAsync(user, MCHRoles.Admin)
            && request.UserRoles.Any(a => !a.Enabled && a.RoleName == MCHRoles.Admin))
        {
            // Get count of users in Admin Role
            int adminCount = (await _userManager.GetUsersInRoleAsync(MCHRoles.Admin)).Count;
