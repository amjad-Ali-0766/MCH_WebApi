using MCH.Infrastructure.Identity;
...
{
    private readonly MCHTenantInfo _currentTenant;
    private readonly RoleManager<ApplicationRole> _roleManager;
    private readonly UserManager<ApplicationUser> _userManager;
    private readonly CustomSeederRunner _seederRunner;
    private readonly ILogger<ApplicationDbSeeder> _logger;

    public ApplicationDbSeeder(MCHTenantInfo currentTenant, RoleManager<ApplicationRole> roleManager, UserManager<ApplicationUser> userManager, CustomSeederRunner seederRunner, ILogger<ApplicationDbSeeder> logger)
    {
    ...
    {
        foreach (string roleName in MCHRoles.DefaultRoles)
        {
            ...
            // Assign permissions
            if (roleName == MCHRoles.Basic)
            {
                await AssignPermissionsToRoleAsync(dbContext, MCHPermissions.Basic, role);
            }
            else if (roleName == MCHRoles.Admin)
            {
                await AssignPermissionsToRoleAsync(dbContext, MCHPermissions.Admin, role);

                if (_currentTenant.Id == MultitenancyConstants.Root.Id)
                {
                    await AssignPermissionsToRoleAsync(dbContext, MCHPermissions.Root, role);
                }
            }
        }
    }

    private async Task AssignPermissionsToRoleAsync(ApplicationDbContext dbContext, IReadOnlyList<MCHPermission> permissions, ApplicationRole role)
    {
        var currentClaims = await _roleManager.GetClaimsAsync(role);
        foreach (var permission in permissions)
        {
            if (!currentClaims.Any(c => c.Type == MCHClaims.Permission && c.Value == permission.Name))
            {
                ...
                {
                    RoleId = role.Id,
                    ClaimType = MCHClaims.Permission,
    ...
    private async Task SeedAdminUserAsync()
    {
        ...
        {
            string adminUserName = $"{_currentTenant.Id.Trim()}.{MCHRoles.Admin}".ToLowerInvariant();
            adminUser = new ApplicationUser
            {
                FirstName = _currentTenant.Id.Trim().ToLowerInvariant(),
                LastName = MCHRoles.Admin,
        ...
        // Assign role to user
        if (!await _userManager.IsInRoleAsync(adminUser, MCHRoles.Admin))
        {
            _logger.LogInformation("Assigning Admin Role to Admin User for '{tenantId}' Tenant.", _currentTenant.Id);
            await _userManager.AddToRoleAsync(adminUser, MCHRoles.Admin);
        }
    }
}