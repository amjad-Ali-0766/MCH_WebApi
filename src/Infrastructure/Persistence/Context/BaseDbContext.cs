...
using MCH.Domain.Common.Contracts;
using MCH.Infrastructure.Auditing;
...
public abstract class BaseDbContext : MultiTenantIdentityDbContext<ApplicationUser, ApplicationRole, string, IdentityUserClaim<string>, IdentityUserRole<string>, IdentityUserLogin<string>, ApplicationRoleClaim, IdentityUserToken<string>>
{
    ...
    private List<AuditTrail> HandleAuditingBeforeSaveChanges(Guid userId)
    {
        ...
        foreach (var entry in ChangeTracker.Entries<IAuditableEntity>().ToList())//"var entry" ma database k table ayin gy jin ma koi changing ho rhi ha
        {
            switch (entry.State)
            {
        ...
        //DetectChanges() is like taking a snapshot of the changes made to entities
        //and  it updates the ChangeTracker with the latest information.
