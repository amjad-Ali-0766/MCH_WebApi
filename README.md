...
- Install using the `MCH CLI` tool. Use this for release versions of the Boilerplate only.
...
After you have installed .NET, you will need to install the `MCH` console tool.

```bash
dotnet tool install --global MCH.CLI
MCH install
```

This isntall the MCH CLI tools and the associated Templates. You are now ready to create your first MCH project!

#### MCH .NET WebAPI Boilerplate
Here's how you would create a Solution using the MCH .NET WebAPI Boilerplate.
...
Run the following command. Note that, in this demonstration, I am naming my new solution as MCH.Starter.

```bash
MCH api new MCH.Starter
```

OR

```bash
MCH api n MCH.Starter
```

This will create a new .NET 7 WEBAPI solution for you using the MCH Templates.
...
```bash
dotnet tool update MCH.CLI --global
MCH update
```
...
There are also examples for mysql & mssql variations of the MCH webapi. You can find the other docker-compose files under the ./docker-compose folder. Read more about [fullstackhero's docker-compose instructions & files here](./docker-compose/README.md)
...
[![MCH dotnet webapi contributors](https://contrib.rocks/image?repo=fullstackhero/dotnet-webapi-boilerplate "MCH dotnet webapi contributors")](https://github.com/fullstackhero/dotnet-webapi-boilerplate/graphs/contributors)
